...
| **The Workbench** | `pages/workbench.html` — the unified tool: one moment runs the **whole engine at once** (the `fullReading` spine) **plus the Vedic chart side by side**, every panel cross-linked via the capability registry, with **JSON / Markdown / SVG / PNG export**, shareable URLs, **silent on-device auto-save** and an optional **publish-to-secret-Gist**, and an **AI assistant** powered by **Claude** (the Anthropic API, called from the browser with your own key) — grounded chat with **in-browser engine tools** (including the Vedic ones), a deep two-system Hermetic **Codex** (with a cross-system "Concordance"), and an agentic **"plan a working"** box. See `WORKBENCH.md` and `docs/LOCAL-LLM.html`. |
